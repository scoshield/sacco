...
class UpdateTransactions
{
    ...
    public function handle($event)
    {
        ...
        foreach ($transactions as &$transaction) {
            ...
            $current_interest = $transaction->interest_repaid_derived;
            $used_up_transaction = $transaction->principal_repaid_derived + $transaction->interest_repaid_derived + $transaction->fees_repaid_derived + $transaction->penalties_repaid_derived;
            ...
            foreach ($repayment_schedules as &$repayment_schedule) {
                ...
                // *************************************************
                $princiapal_less = $repayment_schedule->principal - $repayment_schedule->principal_repaid_derived;
                ...
                if ($loan->loan_transaction_processing_strategy_id == 3) {
                    ...
                    if ($amount >= $interest && $interest > 0) {
                        ...
                        $interest_repaid_derived = $interest_repaid_derived + $interest;
                        $amount = $amount - $interest;
                        // $interest_due = $interest_repaid_derived;
                    } elseif ($amount < $interest && $interest > 0) {
                        $repayment_schedule->interest_repaid_derived = $repayment_schedule->interest_repaid_derived + $amount;
                        $interest_repaid_derived = $interest_repaid_derived + $amount;
                        $amount = 0;
                ...
                $repayment_schedule->total_due=($repayment_schedule->principal - $repayment_schedule->principal_written_off_derived - $repayment_schedule->principal_repaid_derived)+($repayment_schedule->interest - $repayment_schedule->interest_written_off_derived - $repayment_schedule->interest_repaid_derived - $repayment_schedule->interest_waived_derived)+($repayment_schedule->fees - $repayment_schedule->fees_written_off_derived - $repayment_schedule->fees_repaid_derived - $repayment_schedule->fees_waived_derived)+($repayment_schedule->penalties - $repayment_schedule->penalties_written_off_derived - $repayment_schedule->penalties_repaid_derived - $repayment_schedule->penalties_waived_derived);
                $repayment_schedule->principal_balance = $repayment_schedule->principal_balance;
                ...
                // $previous_schedule = $repayment_schedule;
            }         

            // do a journal entry if principal_repaid > 0
            if ($principal_repaid_derived > 0 && $transaction->is_valid == 0) {
                //credit account
                $journal_entry = new JournalEntry();
                $journal_entry->created_by_id = Auth::id();
                $journal_entry->payment_detail_id = $transaction->payment_detail_id;
                $journal_entry->transaction_number = 'L' . $transaction->loan_id;
                ...
                $journal_entry->date = $transaction->created_on;
                $date = explode('-', $transaction->created_on);
                $journal_entry->month = $date[1];
                $journal_entry->year = $date[0];
                $journal_entry->credit = $principal_repaid_derived;
                ...
                if(PaymentType::find(PaymentDetail::find($transaction->payment_detail_id)->payment_type_id)->is_cash == 1)
                {
                    $journal_entry->chart_of_account_id = Auth::user()->user_control_account;
                }else{
                    $journal_entry->chart_of_account_id = PaymentType::find(PaymentDetail::find($transaction->payment_detail_id)->payment_type_id)->asset_control_account;
                }
                $journal_entry->transaction_type = 'loan_repayment';
                $journal_entry->date = $transaction->created_on;
                $date = explode('-', $transaction->created_on);
                $journal_entry->month = $date[1];
                $journal_entry->year = $date[0];
                $journal_entry->debit = $principal_repaid_derived;
                $journal_entry->reference = $loan->id;
                $journal_entry->save();
            }

            // do a journal entry if interest_repaid > 0
            if ($interest_repaid_derived > 0 && $current_interest < $interest_repaid_derived ) {
                //credit account
                $journal_entry = new JournalEntry();
                $journal_entry->created_by_id = Auth::id();
                $journal_entry->payment_detail_id = $transaction->payment_detail_id;
                ...
                $journal_entry->transaction_type = 'loan_interest_repayment';
                $journal_entry->date = $transaction->created_on;
                $date = explode('-', $transaction->created_on);
                $journal_entry->month = $date[1];
                $journal_entry->year = $date[0];
                $journal_entry->credit = $interest_repaid_derived;
                $journal_entry->reference = $loan->id;
                $journal_entry->save();
                ...
                $journal_entry->month = $date[1];
                $journal_entry->year = $date[0];
                $journal_entry->debit = $interest_repaid_derived;
                $journal_entry->reference = $loan->id;
                $journal_entry->save();
        ...
        foreach ($transactions_to_be_updated as $key => $value) {
            ...
            if ($loan->loan_product->accounting_rule == "cash" || $loan->loan_product->accounting_rule == "accrual_periodic" || $loan->loan_product->accounting_rule == "accrual_upfront") {
                ...
                // return response()->json($transaction[5]);
                //principal repaid
                // if ($transaction[4] > 0) {
                //     //credit account
                //     $journal_entry = new JournalEntry();
                ...
                //     $journal_entry->currency_id = $loan->currency_id;
                //     $journal_entry->chart_of_account_id = $loan->loan_product->loan_portfolio_chart_of_account_id;
                //     $journal_entry->transaction_type = 'loan_repayment_credit';
                ...
                //     $journal_entry->currency_id = $loan->currency_id;
                //     $journal_entry->chart_of_account_id = $loan->loan_product->loan_repayment_chart_of_account_id;
                //     $journal_entry->transaction_type = 'loan_repayment_debit';
                //     $journal_entry->date = $transaction[8];
                //     $date = explode('-', $transaction[8]);
                //     $journal_entry->month = $date[1];
                //     $journal_entry->year = $date[0];
